...
def saveToFile(line):
    global inst_count
    # Save the file in the current directory instead of "../cpu/build/"
...
def fillTheFile():
    global FILE_SIZE
    # Use the same file path as in saveToFile
    if not (argList['out_file'] == ''):
        file = argList['out_file']
    else:
        file = argList['inp_file'].split('.')[0] + '.bin'
...
if __name__ == "__main__":
    # Create the instruction dictionary
    read_csv()
    ...
    fillTheFile()
    print(f"Binary file saved to '{os.path.abspath(argList['out_file'] if argList['out_file'] else argList['inp_file'].split('.')[0] + '.bin')}'")