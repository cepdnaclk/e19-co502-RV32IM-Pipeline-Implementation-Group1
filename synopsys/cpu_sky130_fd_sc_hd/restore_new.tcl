...
report_power -group register > "$RESULT_DIR/power_register.txt"
report_power -group sequential > "$RESULT_DIR/power_sequential.txt"
report_power -group combinational > "$RESULT_DIR/power_combinational.txt"
report_power -group black_box > "$RESULT_DIR/power_black_box.txt"
report_power -group io_pad > "$RESULT_DIR/power_io_pad.txt"
...
foreach level $HIERARCHY_LEVELS {
    report_power -hierarchy -levels $level -verbose > "$RESULT_DIR/power_by_module_${level}.txt"
    puts "Generated hierarchical power report for $level levels"
}

# Generate comprehensive hierarchical report
report_power -hierarchy -levels 100 -verbose > "$RESULT_DIR/power_by_module_complete.txt"
...
report_rtl_metrics -list > "$RESULT_DIR/rtl_metrics_list.txt"
...
report_rtl_metrics -view hier \
    ...
    > "$RESULT_DIR/rtl_metrics_hier.txt"
...
report_rtl_metrics -view register \
    ...
    > "$RESULT_DIR/rtl_metrics_register.txt"
...
check_rtl_power > "$RESULT_DIR/check_rtl_power.txt"

# Generate summary report
report_power -verbose > "$RESULT_DIR/power_summary_detailed.txt"
...
set summary_file [open "$RESULT_DIR/power_analysis_summary.txt" w]
